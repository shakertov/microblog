...
def index():
    ...
    if form.validate_on_submit():
    	...
    	db.session.add(post)
    	db.session.commit()
    	flash('Ваша мысль материализовалась!')
...
def login():
	...
	if form.validate_on_submit():
		...
		if user is None or not user.check_password(form.password.data):
			flash('Имя пользователя или пароль не верны.')
...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash('Поздравляем, теперь Вы с нами!')
		return redirect(url_for('login'))
...
def reset_password_request():
	...
	if form.validate_on_submit():
		...
		flash('Проверьте Ваш email - мы отправили Вам инструкцию.')
		return redirect(url_for('login'))
...
def send():
	...
	msg = Message(
		"Hello",
		sender="freedable@yandex.ru",
		recipients=["shakertov@yandex.ru"])
	msg.body = 'Это письмо, отправленное с сайта!'
	mail.send(msg)
